...
result_list = []
proxies = {"http": "http://10.37.235.10:8080", "https": "http://10.37.235.10:8080"}
...
with open("recv_address.txt", "rb") as fp:
    ...
    for ad in addr_list:
        ...
        try:
            result = json.loads(requests.get(_get_url, proxies=proxies).content.decode())
        except Exception:
            continue
        ...
        if result['status'] == 0:
            ...
            result_dict["location"] = result["result"]["address_components"]["province"]\
                                      + result["result"]["address_components"]["city"]\
                                      + result["result"]["address_components"]["district"]\
                                      + result["result"]["address_components"]["street"]
        else:
            result_dict["title"] = ""
            result_dict["lng"] = ""
            result_dict["lat"] = ""
            ...
            result_dict["location"] = result_dict["province"]+result_dict["city"]+result_dict["district"]+result_dict["street"]
